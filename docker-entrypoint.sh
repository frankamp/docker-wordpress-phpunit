...
if [[ "$1" == apache2* ]] || [ "$1" == php-fpm ] || [ "$1" == phpunit ] || [ "$1" == bash ]; then
    ...
    pwd
    echo "checking on wp config next!!!"
...
#shitty test of all-in-one phpunit
#TODO Remove all recipepro bits from this file, provide a hook to setup the tests, or extract
# all of what install-tests is doing in a generic way.
bash /var/www/html/wp-content/plugins/recipe-pro/bin/install-wp-tests.sh wordpress_test root example mysql latest
